...
> **一元 `+` 前缀能干什么？** 它是 C++ 的运算符，所有用途：
>
> | 操作数类型 | `+` 的效果 | 示例 |
> |-----------|-----------|------|
> | 整数 | **整型提升**（小类型→int） | `+short_val` → `int` |
> | 浮点数 | 无操作（值不变） | `+3.14f` → `3.14f` |
> | 指针 | 无操作（值不变） | `+ptr` → `ptr` |
> | 无捕获 lambda | **强制转为函数指针** | `+lambda` → 函数指针 |
> | 枚举 | 整型提升 | `+enum_val` → `int` |
>
> ```cpp
> // 1. 整型提升（最常见的隐式用途）
> short s = 42;
> auto x = +s;    // x 的类型是 int（short 提升为 int）
>
> // 2. 强制 lambda → 函数指针（Blender 用的）
> auto lambda = [](int x) { return x + 1; };
> auto fp = +lambda;  // fp 的类型是 int(*)(int)，不是 lambda
>
> // 3. 枚举提升
...
> // 4. 指针：无操作（合法但无意义）
> int *p = &x;
> auto q = +p;    // q 的类型是 int*，值和 p 相同
> ```
>
> **最常见的用途就是 lambda → 函数指针**。其他用途很少见——整型提升通常隐式发生，不需要显式 `+`。
>
> **为什么 `+lambda` 能转函数指针？** 因为 `+` 只能作用于数值/指针。无捕获 lambda 有一个隐式转换运算符可以转为函数指针。`+` 触发编译器寻找可用的转换——发现 lambda 可以转函数指针，就执行转换。结果类型从 lambda 变成了函数指针。
...
> ```
>
> **为什么不能写 `new(sizeof(T))` 或 `::new(100)`？** `new` 是关键字，不是函数名——关键字没有"调用"语法，后面必须跟类型名：
> ```cpp
> ::new(100);           // ❌ 语法错误！new 后面必须跟类型名
> ::new T(100);         // ✅ 但这是 new 表达式（分配+构造），不是只分配
> new(sizeof(T));       // ❌ 语法错误！
> new T(args);          // ✅ new 表达式
> ::operator new(100);  // ✅ 函数调用——operator new 是函数名
> ```
> `::new T(100)` 和 `::operator new(100)` 完全不同：
> ```cpp
> ::new T(100);           // new 表达式：分配 + 构造 T(100) + 返回 T*
> ::operator new(100);    // 函数调用：只分配 100 字节，返回 void*
> ```
> C++ 没有给 `new` 关键字设计"只分配"的语法——这是语言设计的缺口。想要只分配不构造，只能绕道调用底层函数 `operator new`。
>
> ```cpp
