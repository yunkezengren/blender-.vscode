...
- [Closure to List 节点](#closure-to-list-节点)
  - [目录](#目录)
  - [1. 节点概述与涉及文件](#1-节点概述与涉及文件)
    - [涉及文件完整列表](#涉及文件完整列表)
  - [2. 与 Field to List 的根本区别](#2-与-field-to-list-的根本区别)
  - [3. DNA 存储结构详解](#3-dna-存储结构详解)
    - [GeometryNodeClosureToListItem](#geometrynodeclosuretolistitem)
    - [GeometryNodeClosureToList](#geometrynodeclosuretolist)
    - [与 FieldToList DNA 的对比](#与-fieldtolist-dna-的对比)
  - [4. ClosureToListItemsAccessor 详解](#4-closuretolistitemsaccessor-详解)
    - [完整定义](#完整定义)
    - [关键常量与方法详解](#关键常量与方法详解)
      - [`has_single_identifier_str = false`](#has_single_identifier_str--false)
      - [`input_socket_identifier_for_item` 返回空字符串](#input_socket_identifier_for_item-返回空字符串)
      - [`supports_socket_type` — 支持所有几何节点类型](#supports_socket_type--支持所有几何节点类型)
      - [`copy_item` — 深拷贝名称](#copy_item--深拷贝名称)
      - [`destruct_item` — 安全释放名称](#destruct_item--安全释放名称)
    - [Accessor 与 FieldToListItemsAccessor 对比](#accessor-与-fieldtolistitemsaccessor-对比)
  - [5. 节点声明 — 闭包签名](#5-节点声明--闭包签名)
    - [Socket 布局](#socket-布局)
    - [声明顺序的重要性](#声明顺序的重要性)
  - [6. 节点 UI 布局与 structure\_type 属性](#6-节点-ui-布局与-structure_type-属性)
  - [7. 核心执行逻辑](#7-核心执行逻辑)
    - [结果数组分配](#结果数组分配)
  ...
  - [9. evaluate\_closure\_eagerly 内部机制](#9-evaluate_closure_eagerly-内部机制)
    - [ClosureEagerEvalParams 结构](#closureeagerevalparams-结构)
    - [evaluate\_closure\_eagerly 执行流程](#evaluate_closure_eagerly-执行流程)
    - [关键步骤详解](#关键步骤详解)
      - [1. 输入/输出映射](#1-输入输出映射)
      - [2. 隐式类型转换](#2-隐式类型转换)
      - [3. 就地构造输出](#3-就地构造输出)
  ...
  - [12. 节点生命周期函数](#12-节点生命周期函数)
    - [node\_init — 初始化](#node_init--初始化)
    - [node\_free\_storage — 释放](#node_free_storage--释放)
    - [node\_copy\_storage — 复制](#node_copy_storage--复制)
    - [node\_blend\_write / node\_blend\_read — 序列化](#node_blend_write--node_blend_read--序列化)
    - [Accessor 的 blend\_write\_item / blend\_read\_data\_item](#accessor-的-blend_write_item--blend_read_data_item)
    - [node\_internally\_linked\_input — 内部链接](#node_internally_linked_input--内部链接)
    - [node\_insert\_link — 拖拽添加项](#node_insert_link--拖拽添加项)
    - [生命周期函数总览](#生命周期函数总览)
  ...
  - [14. Socket 自动同步机制](#14-socket-自动同步机制)
    - [同步状态枚举](#同步状态枚举)
    - [ClosureSyncState 结构](#closuresyncstate-结构)
    - [get\_sync\_state\_closure\_to\_list](#get_sync_state_closure_to_list)
    - [sync\_sockets\_closure\_to\_list — 执行同步](#sync_sockets_closure_to_list--执行同步)
    - [同步触发时机](#同步触发时机)
  - [15. 同步系统完整调用链](#15-同步系统完整调用链)
    - [sync\_node — 同步入口](#sync_node--同步入口)
    - [node\_can\_sync\_sockets — 判断是否可同步](#node_can_sync_sockets--判断是否可同步)
    - [sync\_node\_description\_get — 同步提示文本](#sync_node_description_get--同步提示文本)
    - [完整调用链](#完整调用链)
  - [16. references\_other\_outputs 与声明系统变更](#16-references_other_outputs-与声明系统变更)
    - [新增声明](#新增声明)
    - [实现](#实现-1)
    - [重构 anonymous\_attribute\_output](#重构-anonymous_attribute_output)
    - [Closure to List 中的使用](#closure-to-list-中的使用)
  - [17. 链接搜索与节点注册](#17-链接搜索与节点注册)
    - [链接搜索](#链接搜索)
    - [注册](#注册)
    - [注册回调详解](#注册回调详解)
  - [18. RNA 定义完整分析](#18-rna-定义完整分析)
    - [rna\_GeometryNodeClosureToListItem\_structure\_type\_itemf — 动态枚举过滤](#rna_geometrynodeclosuretolistitem_structure_type_itemf--动态枚举过滤)
    - [rna\_def\_geo\_closure\_to\_list\_item — 项 RNA](#rna_def_geo_closure_to_list_item--项-rna)
    - [rna\_def\_geo\_closure\_to\_list\_items — 集合 RNA](#rna_def_geo_closure_to_list_items--集合-rna)
    - [def\_geo\_closure\_to\_list — 节点 RNA](#def_geo_closure_to_list--节点-rna)
    - [Python API 使用示例](#python-api-使用示例)
  - [19. trace\_values 与值追踪集成](#19-trace_values-与值追踪集成)
    - [修改内容](#修改内容)
    - [值追踪的作用](#值追踪的作用)
  - [20. 完整执行流程端到端追踪](#20-完整执行流程端到端追踪)
    - [场景设定](#场景设定)
    - [端到端流程](#端到端流程)
    - [单次闭包执行详解（list\_i=1）](#单次闭包执行详解list_i1)
    - [Count=0 的特殊情况](#count0-的特殊情况)
  - [21. 提交历史](#21-提交历史)
    - [提交中的关键设计决策](#提交中的关键设计决策)
