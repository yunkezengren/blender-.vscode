...
- [列表函数求值系统](#列表函数求值系统)
  ...
  - [3. execute\_multi\_function\_on\_value\_variant\_\_list — 核心执行器](#3-execute_multi_function_on_value_variant__list--核心执行器)
    - [完整执行流程](#完整执行流程)
    - [步骤 1：找到最长列表长度](#步骤-1找到最长列表长度)
  - [4. 输入处理的三种模式](#4-输入处理的三种模式)
    - [单值 → 直接作为常量输入](#单值--直接作为常量输入)
    - [列表 → 重复扩展](#列表--重复扩展)
    - [字段 → 先求值为列表](#字段--先求值为列表)
  - [5. create\_repeated\_list — 列表重复扩展](#5-create_repeated_list--列表重复扩展)
  - [6. add\_list\_to\_params — 列表到参数的映射](#6-add_list_to_params--列表到参数的映射)
  - [7. 输出处理与 GListPtr 包装](#7-输出处理与-glistptr-包装)
  - [8. 完整数据流追踪](#8-完整数据流追踪)
    - [内存布局](#内存布局)
  - [9. ListFieldContext — 列表字段上下文](#9-listfieldcontext--列表字段上下文)
    - [9.1 为什么需要 ListFieldContext？](#91-为什么需要-listfieldcontext)
    - [9.2 类定义与实现](#92-类定义与实现)
    - [9.3 工作原理](#93-工作原理)
    - [9.4 使用场景](#94-使用场景)
  - [10. SampleIndexFunction — 列表到字段的桥梁](#10-sampleindexfunction--列表到字段的桥梁)
    ...
    - [10.2 类定义](#102-类定义)
    - [10.3 构造函数](#103-构造函数)
    - [10.4 call 方法详解](#104-call-方法详解)
    - [10.5 关键语法解释](#105-关键语法解释)
    - [10.6 hash\_unique 方法](#106-hash_unique-方法)
    - [10.7 两个使用场景](#107-两个使用场景)
      - [场景 1：Sort List 节点 — 列表→字段适配](#场景-1sort-list-节点--列表字段适配)
      - [场景 2：Get List Item 节点 — 按索引取值](#场景-2get-list-item-节点--按索引取值)
    - [10.8 与 bke::SampleIndexFunction 的区别](#108-与-bkesampleindexfunction-的区别)
  - [11. evaluate\_field\_to\_list — 字段到列表的桥梁](#11-evaluate_field_to_list--字段到列表的桥梁)
    - [11.1 函数签名](#111-函数签名)
    - [11.2 实现](#112-实现)
    - [11.3 数据流](#113-数据流)
    - [11.4 使用场景](#114-使用场景)
  - [12. 两个方向的完整对照](#12-两个方向的完整对照)
    - [12.1 为什么需要两个方向？](#121-为什么需要两个方向)
    - [12.2 list\_function\_eval.hh 完整 API 一览](#122-list_function_evalhh-完整-api-一览)
  - [13. 调度入口 — `execute_multi_function_on_value_variant`](#13-调度入口--execute_multi_function_on_value_variant)
    - [13.1 统一入口函数](#131-统一入口函数)
    - [13.2 调度优先级](#132-调度优先级)
    - [13.3 四条路径对比](#133-四条路径对比)
  - [14. `execute_multi_function_on_value_variant__list` 完整详解](#14-execute_multi_function_on_value_variant__list-完整详解)
    - [14.1 函数签名](#141-函数签名)
    - [14.2 完整代码逐行解析](#142-完整代码逐行解析)
    - [14.3 关键设计决策](#143-关键设计决策)
    - [14.4 完整示例：两个浮点列表相加](#144-完整示例两个浮点列表相加)
