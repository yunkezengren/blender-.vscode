...
```cpp
/* 示例1: 创建普通资源句柄 (索引=42, 不反转) */
uint32_t raw = 42;  // 0x0000002A
// 内存布局: 00000000 00000000 00000000 00101010
// 值: 42

/* 示例2: 创建反转手性句柄 (索引=42, 反转) */
uint32_t raw = 42 | 0x80000000;  // 0x8000002A
// 内存布局: 10000000 00000000 00000000 00101010
// 值: 2,147,483,690 (2^31 + 42)

/* 示例3: 解码 */
uint32_t handle = 0x8000002A;
bool inverted = (handle & 0x80000000) != 0;  // true
uint32_t index = handle & 0x7FFFFFFF;        // 42

/* 示例4: 边界情况 - 最大索引 */
uint32_t max_index = 0x7FFFFFFF;  // 2,147,483,647
uint32_t max_inverted = 0xFFFFFFFF;  // 所有位都为1
// 解码后: index = 0x7FFFFFFF, inverted = true
```
...
反转资源 (索引=100):
  二进制: 10000000 00000000 00000000 01100100
  十六进制: 0x80000064
  索引: 100, 反转: 是
  十进制值: 2,147,483,748
...
```cpp
struct ResourceIndex {
  uint32_t raw;  // 完全可控的32位整数

  // 构造函数
  ResourceIndex(uint index, bool inverted) {
    raw = index;
    if (inverted) {
      raw |= 0x80000000u;  // 设置最高位
    }
  }

  // 访问器
  uint index() const { return raw & 0x7FFFFFFFu; }
  bool inverted() const { return (raw & 0x80000000u) != 0; }
...
### SET_FLAG_FROM_TEST 宏解析

**源码位置**: `BLI_utildefines.h`

```cpp
#define SET_FLAG_FROM_TEST(var, condition, flag) \
  (var) = ((var) & ~(flag)) | ((-(int)(condition)) & (flag))
```

**工作原理**:
```cpp
condition = true:
  -(int)true = -1 = 0xFFFFFFFF (所有位为1)
  (var) & ~(flag)      → 清除指定标志位
  0xFFFFFFFF & (flag)  → 保留flag中的位
  | 操作 → 设置标志
...
```cpp
// 当 condition = true
raw = (raw & ~0x80000000u) | (0xFFFFFFFF & 0x80000000u)
    = (raw & 0x7FFFFFFFu) | 0x80000000u
    = (raw & 0x7FFFFFFFu) | 0x80000000u  // 最高位置1

// 当 condition = false
raw = (raw & ~0x80000000u) | (0 & 0x80000000u)
    = (raw & 0x7FFFFFFFu) | 0
    = raw & 0x7FFFFFFFu  // 最高位清零
```

这个宏是**branchless**的，在性能关键路径上比if语句更优。
