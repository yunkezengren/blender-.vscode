# 为什么使用 `std::shared_ptr`？GeometryComponent 和 GeometrySet 的区别？为什么需要类型转换？
...
1. **从基类到派生类**
   - `GeometryComponent` 是基类，没有派生类的特有方法
   - 需要转换为派生类才能访问特有方法
...
3. **类型安全**
   - 通过 `component.type()` 检查类型
   - 已知类型时，`static_cast` 是安全的

4. **模板方法的便利性**
   - `get_component<MeshComponent>()` 自动转换
   - 类型安全，代码简洁
