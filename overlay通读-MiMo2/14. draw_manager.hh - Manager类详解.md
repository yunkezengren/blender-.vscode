...
class Manager {
 protected:
  ...
  using MatrixBuf = UniformArrayBuffer<ObjectMatrix, DRW_RESOURCE_CHUNK_SIZE>;

  using BoundsBuf = UniformArrayBuffer<ObjectBounds, DRW_RESOURCE_CHUNK_SIZE>;

  using InfosBuf = UniformArrayBuffer<ObjectInfos, DRW_RESOURCE_CHUNK_SIZE>;

  using AttrBuf = StorageArrayBuffer<ObjectAttribute, DRW_ATTRIBUTE_CHUNK_SIZE>;

  using LayerAttrBuf = UniformArrayBuffer<LayerAttribute, DRW_LAYER_ATTR_MAX>;

  MatrixBuf matrix_buf_;        // 对象矩阵 (4x4 float)
  BoundsBuf bounds_buf_;        // 包围盒 (中心+半边长)
  InfosBuf infos_buf_;          // 对象信息 (ID, 状态标志, 属性偏移)
  AttrBuf attributes_buf_;      // 材质属性 (颜色, 透明度等)
  LayerAttrBuf layer_attributes_buf_; // 图层属性 (层名, 类型等)
