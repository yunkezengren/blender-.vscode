...
<span style="background-color:#FFA500;color:black;font-weight:bold">Brick Texture</span> 是 Blender 中用于生成程序化砖墙纹理的着色器节点。它<span style="background-color:#7CFC00;color:black;font-weight:bold">无需外部贴图</span>，纯数学算法即可创建逼真的砖墙图案，支持多种参数控制砖块布局、颜色、灰缝等特征。
...
graph TD
    subgraph "用户输入"
        A[Vector<br/>坐标输入] --> B[Scale<br/>缩放]
        C[Color1<br/>主砖颜色] --> D[混合器]
        E[Color2<br/>次砖颜色] --> D
        F[Mortar<br/>灰缝颜色] --> G[最终混合]
    end

    subgraph "核心计算"
        B --> H[brick()函数]
        H --> I[行号计算<br/>rownum]
        H --> J[列号计算<br/>bricknum]
        I --> K[偏移/压缩判断]
        J --> K
        K --> L[局部坐标<br/>x, y]
        L --> M[噪声函数<br/>tint]
        L --> N[灰缝距离<br/>min_dist]
        N --> O[smoothstep<br/>边缘平滑]
    end
    ...
    style Q fill:#7CFC00,stroke:#333,stroke-width:2px
```
